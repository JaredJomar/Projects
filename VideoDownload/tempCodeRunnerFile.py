...
class DownloadThread(threading.Thread):
    ...
    def download_video(self, url):
        if self.download_type == "video":
            command = [self.yt_dlp_path, "--format", "bestvideo+bestaudio/best", "--merge-output-format", "mkv",
                       "-o", f"{self.output_folder}/%(title)s.%(ext)s", "--ffmpeg-location", self.ffmpeg_path, url]
        elif self.download_type == "audio":
            command = [self.yt_dlp_path, "--format", "bestaudio/best", "-x", "--audio-format", "mp3",
                       "-o", f"{self.output_folder}/%(title)s.%(ext)s", "--ffmpeg-location", self.ffmpeg_path, url]
    ...
    def run(self):
        ...
        for index, url in enumerate(self.urls):
            ...
            self.main_window.update_progress_text(
                f"Downloading {url} ({index + 1}/{total_urls})")
            self.queue.put(url)

            if len(download_threads) < self.max_threads:
                url_to_download = self.queue.get()
                download_thread = threading.Thread(
                    target=self.download_video, args=(url_to_download,))
...
class MainWindow(tk.Tk):
    ...
    def create_main_tab(self, tab):
        label_url = ttk.Label(tab, text="URL:", background="#06283D",
                              foreground="white", font=("Arial", 10, "bold"))
        label_url.grid(row=0, column=0, sticky="e", padx=5, pady=5)

        self.url_input = ttk.Entry(tab, font=("Arial", 10, "bold"))
        self.url_input.grid(row=0, column=1, columnspan=2,
                            sticky="nsew", padx=5, pady=5)

        browse_text_file_button = ttk.Button(
            tab, text="Browse Text File", command=self.browse_text_file, style="TButton", cursor="hand2")
        browse_text_file_button.grid(
            row=0, column=3, sticky="w", padx=5, pady=5)

        start_button = ttk.Button(
            tab, text="Download", command=self.start_download, style="TButton", cursor="hand2")
        start_button.grid(row=0, column=5, sticky="w", padx=5, pady=5)

        cancel_button = ttk.Button(
            tab, text="Cancel", command=self.cancel_download, style="TButton", cursor="hand2")
        cancel_button.grid(row=0, column=6, sticky="w", padx=5, pady=5)

        label_download_folder = ttk.Label(
            tab, text="Download Path:", background="#06283D", foreground="white", font=("Arial", 10, "bold"))
        ...
        self.download_folder_input.grid(
            row=1, column=1, columnspan=2, sticky="nsew", padx=5, pady=5)

        browse_download_folder_button = ttk.Button(
            tab, text="Browse", command=self.browse_download_folder, style="TButton", cursor="hand2")
        browse_download_folder_button.grid(
            row=1, column=3, sticky="w", padx=5, pady=5)

        label_download_type = ttk.Label(
            tab, text="Download Type:", background="#06283D", foreground="white", font=("Arial", 10, "bold"))
        label_download_type.grid(row=2, column=0, sticky="e", padx=5, pady=5)

        self.download_type_combobox = ttk.Combobox(
            tab, values=["video", "audio", "video with audio"], font=("Arial", 10, "bold"))
        # Set "video with audio" as the default
        self.download_type_combobox.current(2)
        self.download_type_combobox.grid(
            row=2, column=1, columnspan=2, sticky="nsew", padx=5, pady=5)

        label_resolution = ttk.Label(
            tab, text="Resolution:", background="#06283D", foreground="white", font=("Arial", 10, "bold"))
        label_resolution.grid(row=3, column=0, sticky="e", padx=5, pady=5)

        self.resolution_combobox = ttk.Combobox(tab, values=[
                                                "240p", "360p", "480p", "720p", "1080p", "best"], font=("Arial", 10, "bold"))
        ...
        self.progress_text = tk.Text(
            tab, height=5, width=50, font=("Arial", 10, "bold"))
        self.progress_text.grid(
            row=4, column=0, columnspan=7, sticky="nsew", padx=5, pady=5)
    ...
    def start_download(self):
        ...
        self.download_thread = DownloadThread(
            url, output_folder, ffmpeg_path, yt_dlp_path, download_type, resolution, self)
    ...
    def load_settings(self):
        if os.path.exists('settings.json'):
            ...
            if not self.download_folder_input.get():
                self.ffmpeg_input.insert(
                    0, self.settings.get('ffmpeg_path', ''))
                self.yt_dlp_input.insert(
                    0, self.settings.get('yt_dlp_path', ''))
                self.download_folder_input.insert(
                    0, self.settings.get('download_folder', ''))
